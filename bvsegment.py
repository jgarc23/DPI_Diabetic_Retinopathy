...
def extract_bv(image):
    ...
    for cnt in xcontours:
        ...
        if len(approx) > 4 and 100 <= cv2.contourArea(cnt) <= 3000:
...
if __name__ == "__main__":
    ...
    pathFolder = r"C:\Users\Yoona\Downloads\opencv\Image Segmentation\input_folder"
    destinationFolder = r"C:\Users\Yoona\Downloads\opencv\Image Segmentation\output_folder"
    ...
    for file_name in filesArray:
        ...
        fundus = cv2.imread(os.path.join(pathFolder, file_name))
        bloodvessel = extract_bv(fundus)
        cv2.imwrite(os.path.join(destinationFolder, f"{file_name_no_extension}_bloodvessel.png"), bloodvessel)
